...
# สร้าง Blueprint สำหรับ Lab 8
lab8_bp = Blueprint('lab8', __name__)

# โฟลเดอร์เก็บไฟล์เฉลย (config) ของ lab8
...
scores_collection = db['lab8_scores']  # แยก Collection สำหรับ Lab 2
...
def check_config_lab8():
    """
    รับค่าการตั้งค่า Switch, PC จากฟอร์มใน lab8.html
    ตรวจสอบความถูกต้อง
    บันทึกคะแนนลงฐานข้อมูล (เป็นเปอร์เซ็นต์)
    ส่งผลลัพธ์ไปแสดงใน lab8.html
    """
    # รับค่าที่ผู้ใช้กรอก
    user_switch_config = request.form.get('config_switch', '').strip()
    user_pc_config = request.form.get('config_pc', '').strip()

    # อ่านค่าที่ถูกต้องจากไฟล์ (อยู่ใน check_config/lab8_sw1.txt, lab8_pc1.txt)
    correct_switch_config = read_config_file('lab8_sw1.txt')
    correct_pc_config = read_config_file('lab8_pc1.txt')
