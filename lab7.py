...
# สร้าง Blueprint สำหรับ Lab 7
lab7_bp = Blueprint('lab7', __name__)

# โฟลเดอร์เก็บไฟล์เฉลย (config) ของ lab7
...
scores_collection = db['lab7_scores']  # แยก Collection สำหรับ Lab 2
...
def check_config_lab7():
    """
    รับค่าการตั้งค่า Switch, PC จากฟอร์มใน lab7.html
    ตรวจสอบความถูกต้อง
    บันทึกคะแนนลงฐานข้อมูล (เป็นเปอร์เซ็นต์)
    ส่งผลลัพธ์ไปแสดงใน lab7.html
    """
    # รับค่าที่ผู้ใช้กรอก
    user_switch_config = request.form.get('config_switch', '').strip()
    user_pc_config = request.form.get('config_pc', '').strip()

    # อ่านค่าที่ถูกต้องจากไฟล์ (อยู่ใน check_config/lab7_sw1.txt, lab7_pc1.txt)
    correct_switch_config = read_config_file('lab7_sw1.txt')
    correct_pc_config = read_config_file('lab7_pc1.txt')
